...
-- Drop budget_category_subcategories table (cascade will handle foreign keys)
DROP TABLE IF EXISTS "budget_category_subcategories";

-- Remove subcategoryId column from transaction_categories
-- First, set any existing subcategoryId values to NULL (they'll be invalid after dropping the table anyway)
